...
use std::{
  ...
  time::{Duration, SystemTime, Instant}, collections::VecDeque,
};
...
impl NoDelaySendQueue {
  ...
  fn push_frames(&mut self, frames: &[GameStreamFrame]) {
    ...
    for frame in frames {
      self
        .q
        .push_back(Frame::new_bytes(PacketTypeId::ObserverData, frame.data.clone()));
    }
    if !self.q.is_empty() {
...
impl DelaySendQueue {
  ...
  pub fn push_frames(&mut self, frames: &[GameStreamFrame]) {
    ...
    for frame in frames {
      let fast_forwarding = now.saturating_sub(frame.approx_timestamp_millis) <= self.fast_forwarding_millis;
      let delay_millis = if fast_forwarding {
        (self.delay_millis as f64 / flo_constants::OBSERVER_FAST_FORWARDING_SPEED).round() as i64
      } else {
        self.delay_millis
      };
      ...
      self
          .delayed
          .insert((self.next_seq, Frame::new_bytes(PacketTypeId::ObserverData, frame.data.clone())),  delay);
